...
import { ConfirmDialog, Icon, Modal } from "@/components/common";
...
export function ItemDetailsModal({
  ...
  const deleteTitle = item?.title?.trim() || "this item";
    ...
    <>
      <Modal
        ...
        description={sourceLabel(
          item ?? ({ source: "DONNA", type: "EVENT" } as TimelineItem),
        )}
      >
        {item ? (
          <div className="space-y-4">
            <dl className="space-y-3 text-sm">
              ...
              <Row label="Type">
                {item.type === "REMINDER" ? "Reminder" : "Event"}
              </Row>
